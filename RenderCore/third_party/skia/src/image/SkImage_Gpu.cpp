...
bool SkImage_Gpu::onReadPixels(const SkImageInfo& dstInfo, void* dstPixels, size_t dstRB,
    ...
    // eglSureface (rather than the opposite), and our lenient image rules break our (currently) more
    // strict eglSureface rules.
    // GrSurfaceContext::readPixels does not make use of the context's color space. However, we
    // don't allow creating a eglSureface context for a sRGB GrPixelConfig unless the color space has
