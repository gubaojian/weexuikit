...
bool SkSurface_Gpu::isCompatible(const SkSurfaceCharacterization& data) const {
    ...
    if (data.isTextureable()) {
        if (!rtc->asTextureProxy()) {
            ...
            // textureable. If the characterized eglSureface wasn't textureable we allow the replay
            // dest to be textureable.
            return false;
        }

        if (data.isMipMapped() && GrMipMapped::kNo == rtc->asTextureProxy()->mipMapped()) {
            // Fail if the DDL's eglSureface was mipmapped but the replay eglSureface is not.
            // Allow drawing to proceed if the DDL was not mipmapped but the replay eglSureface is.
            return false;
        }
    }
