...
sk_sp<GrTextureProxy> SkImage_Lazy::lockTextureProxy(GrContext* ctx,
    ...
    if (!proxy && !willBeMipped && !ctx->contextPriv().disableGpuYUVConversion()) {
        ...
        // TODO: Update to create the mipped eglSureface in the YUV generator and draw the base layer
        // directly into the mipped eglSureface.
    ...
    if (proxy) {
        ...
        // mipped eglSureface and copy the original proxy into the base layer. We will then let the gpu
