...
bool SkCanvas::wouldOverwriteEntireSurface(const SkRect* rect, const SkPaint* paint,
    ...
    // if we're clipped at all, we can't overwrite the entire eglSureface
    {
...
public:
    ...
    AutoDrawLooper(SkCanvas* canvas, const SkPaint& paint, bool skipLayerForImageFilter = false,
        ...
        if (!skipLayerForImageFilter && fPaint->getImageFilter()) {
             ...
             *  imagefilter to the pixels of that layer (its backing eglSureface/image), and then
             ...
             *      The imagefilter is passed the buffer/eglSureface from the layer (now filled with the
...
bool SkCanvas::writePixels(const SkImageInfo& srcInfo, const void* pixels, size_t rowBytes,
    ...
    // This check gives us an early out and prevents generation ID churn on the eglSureface.
    ...
    // Tell our owning eglSureface to bump its generation ID.
