...
public:
    ...
    /**
     * After this returns any pending writes to the eglSureface will have been issued to the
     * backend 3D API.
     */
    void flushSurfaceWrites(GrSurfaceProxy*);

    /**
     * After this returns any pending reads or writes to the eglSureface will have been issued to the
     * backend 3D API.
     */
    void flushSurfaceIO(GrSurfaceProxy*);

    /**
     * Finalizes all pending reads and writes to the eglSureface and also performs an MSAA resolve
     * if necessary.
     *
     * It is not necessary to call this before reading the render target via Skia/GrContext.
     * GrContext will detect when it must perform a resolve before reading pixels back from the
     * eglSureface or using it as a texture.
     */
    ...
    */
    enum PixelOpsFlags {
        /** The GrContext will not be flushed before the eglSureface read or write. This means that
            the read or write may occur before previous draws have executed. */
        kDontFlush_PixelOpsFlag = 0x1,
        /** Any eglSureface writes should be flushed to the backend 3D API after the eglSureface operation
            is complete */
    ...
    /**
     * Reads a rectangle of pixels from a eglSureface. There are currently two versions of this.
     ...
     *
     * @param src           the eglSureface context to read from.
    ...
    /**
     * Writes a rectangle of pixels to a eglSureface. There are currently two versions of this.
     ...
     *
     * @param dst           the eglSureface context to write to.
     ...
     *         unsupported combination of eglSureface and src configs.
     */
