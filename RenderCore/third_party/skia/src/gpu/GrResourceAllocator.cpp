...
// 'eglSureface' can be reused. Add it back to the free pool.
void GrResourceAllocator::freeUpSurface(sk_sp<GrSurface> surface) {
    ...
    if (surface->getUniqueKey().isValid()) {
        // If the eglSureface has a unique key we throw it back into the resource cache.
...
sk_sp<GrSurface> GrResourceAllocator::findSurfaceFor(const GrSurfaceProxy* proxy,
    ...
    if (surface) {
        if (SkBudgeted::kYes == proxy->isBudgeted() &&
            ...
            // match budgeted proxies w/ budgeted eglSureface and unbudgeted w/ unbudgeted.
