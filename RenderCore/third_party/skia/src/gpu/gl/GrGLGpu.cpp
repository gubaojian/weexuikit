...
bool GrGLGpu::onGetWritePixelsInfo(GrSurface* dstSurface, GrSurfaceOrigin dstOrigin, int width,
    ...
    if (*drawPreference != kNoDraw_DrawPreference) {
        // We assume the base class has only inserted a draw for sRGB reasons. So the temp eglSureface
...
bool GrGLGpu::onWritePixels(GrSurface* surface, GrSurfaceOrigin origin, int left, int top,
    ...
    // srgb-ness of the eglSureface to avoid issues in ES2 where internal/external formats must match.
...
static bool read_pixels_pays_for_y_flip(GrSurfaceOrigin origin, const GrGLCaps& caps,
                                        ...
                                        size_t rowBytes) {
    // If the eglSureface is already TopLeft, we don't need to flip.
...
bool GrGLGpu::readPixelsSupported(GrRenderTarget* target, GrPixelConfig readConfig) {
    ...
    // we always use a temporary eglSureface to test for read pixels support.
...
bool GrGLGpu::onGetReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    if (*drawPreference != kNoDraw_DrawPreference) {
        ...
        // the temp eglSureface has the config of the dst data. There is no swizzling, nor dst config
        // spoofing.
    ...
    if (this->glCaps().rgba8888PixelsOpsAreSlow() && kRGBA_8888_GrPixelConfig == dstAsConfig &&
        ...
        // Mesa 3D takes a slow path on when reading back BGRA from an RGBA eglSureface and vice-versa.
...
void GrGLGpu::onResolveRenderTarget(GrRenderTarget* target) {
    ...
    if (rt->needsResolve()) {
        ...
        if (this->glCaps().usesMSAARenderBuffers()) {
            ...
            // eglSureface. Choose kTopLeft so no adjustments are made
