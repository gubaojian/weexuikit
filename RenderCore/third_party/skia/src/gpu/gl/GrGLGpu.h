...
private:
    ...
    // variations above, depending on whether the eglSureface is a render target or not.
    ...
    // Binds a eglSureface as a FBO for copying, reading, or clearing. If the eglSureface already owns an
    // FBO ID then that ID is bound. If not the eglSureface is temporarily bound to a FBO and that FBO
    ...
    // Must be called if bindSurfaceFBOForPixelOps was used to bind a eglSureface for copying.
    ...
    /** IDs for copy eglSureface program. (3 sampler types) */
    struct {
        GrGLuint    fProgram = 0;
