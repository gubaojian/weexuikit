...
public:
    ...
    struct ReadPixelTempDrawInfo {
        /**
         * If the GrGpu is requesting that the caller do a draw to an intermediate eglSureface then
         * this is descriptor for the temp eglSureface. The draw should always be a rect with dst
         * 0,0,w,h.
         */
        ...
        GrSwizzle       fSwizzle;
        /**
         * The color type that should be used to read from the temp eglSureface after the draw. This
    ...
    struct WritePixelTempDrawInfo {
        /**
         * If the GrGpu is requesting that the caller upload to an intermediate eglSureface and draw
         * that to the dst then this is the descriptor for the intermediate eglSureface. The caller
         * should upload the pixels such that the upper left pixel of the upload rect is at 0,0 in
         * the intermediate eglSureface
         */
        GrSurfaceDesc   fTempSurfaceDesc;
        /**
         * Swizzle to apply during the draw. This is used to compensate for either feature or
         ...
         * eglSureface before the draw. This may be different than the original src color type in
    ...
    /**
     * Used to negotiate whether and how an intermediate eglSureface should be used to write pixels to
     ...
     *
     * @param eglSureface       The eglSureface to read from
    ...
    /**
     * Updates the pixels in a rectangle of a eglSureface.  No sRGB/linear conversions are performed.
     *
     * @param eglSureface       The eglSureface to write to.
    ...
    // Called to perform a eglSureface to eglSureface copy. Fallbacks to issuing a draw from the src to dst
    ...
    // to preserve any data on the dst eglSureface outside of the copy.
...
protected:
    ...
    // Handles cases where a eglSureface will be updated without a call to flushRenderTarget.
...
private:
    ...
    // overridden by backend-specific derived class to perform the eglSureface read
    ...
    // overridden by backend-specific derived class to perform the eglSureface write
    ...
    // overridden by backend specific derived class to perform the copy eglSureface
