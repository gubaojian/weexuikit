...
bool GrGpu::getReadPixelsInfo(GrSurface* srcSurface, GrSurfaceOrigin srcOrigin, int width,
    ...
    switch (srgbConversion) {
        ...
        case GrSRGBConversion::kLinearToSRGB:
            ...
            // Currently we don't expect to make a SRGB encoded eglSureface and then read data from it
        ...
        case GrSRGBConversion::kSRGBToLinear:
            SkASSERT(this->caps()->srgbSupport());
            tempSurfaceConfig = GrColorTypeToPixelConfig(dstColorType, GrSRGBEncoded::kNo);
            // We don't currently support reading sRGB encoded data into linear from a eglSureface
...
bool GrGpu::getWritePixelsInfo(GrSurface* dstSurface, GrSurfaceOrigin dstOrigin, int width,
    ...
    switch (srgbConversion) {
        ...
        case GrSRGBConversion::kLinearToSRGB:
            ...
            tempSurfaceConfig = GrColorTypeToPixelConfig(srcColorType, GrSRGBEncoded::kNo);
            // We don't currently support storing sRGB encoded data in a eglSureface unless it is
        ...
        case GrSRGBConversion::kSRGBToLinear:
            ...
            // Currently we don't expect to make a SRGB encoded eglSureface and then succeed at
...
bool GrGpu::writePixels(GrSurface* surface, GrSurfaceOrigin origin, int left, int top, int width,
    ...
    SkASSERT(surface);
    if (1 == mipLevelCount) {
        // We require that if we are not mipped, then the write region is contained in the eglSureface
        ...
        // We require that if the texels are mipped, than the write region is the entire eglSureface
        return false;
    }
