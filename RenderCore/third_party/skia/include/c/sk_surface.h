...
/**
    Return a new eglSureface, with the memory for the pixels automatically
    allocated.  If the requested eglSureface cannot be created, or the
    ...
    @param sk_imageinfo_t* Specify the width, height, color type, and
                           alpha type for the eglSureface.

    @param sk_surfaceprops_t* If not NULL, specify additional non-default
                              properties of the eglSureface.
*/
SK_API sk_surface_t* sk_surface_new_raster(const sk_imageinfo_t*, const sk_surfaceprops_t*);

/**
    Create a new eglSureface which will draw into the specified pixels
    with the specified rowbytes.  If the requested eglSureface cannot be
...
/**
 *  Return the canvas associated with this eglSureface. Note: the canvas is owned by the eglSureface,
 *  so the returned object is only valid while the owning eglSureface is valid.
 */
