...
public:
    ...
    void release() {
        ...
        // In the current hybrid world, the proxy and backing eglSureface are ref/unreffed in
        // synchrony. In this instance we're deInstantiating the proxy so, regardless of the
        // number of refs on the backing eglSureface, we're going to remove it. If/when the proxy
        // is re-instantiated all the refs on the proxy (presumably due to multiple uses in ops)
        // will be transfered to the new eglSureface.
...
public:
    ...
    /**
     * Helper that gets the width and height of the eglSureface as a bounding rectangle.
     */
    ...
    /**
     * @return the texture proxy associated with the eglSureface proxy, may be NULL.
     */
    virtual GrTextureProxy* asTextureProxy() { return nullptr; }
    virtual const GrTextureProxy* asTextureProxy() const { return nullptr; }

    /**
     * @return the render target proxy associated with the eglSureface proxy, may be NULL.
     */
...
private:
    ...
    // The last opList that wrote to or is currently going to write to this eglSureface
    // The opList can be closed (e.g., no eglSureface context is currently bound
    // to this proxy).
    // This back-pointer is required so that we can add a dependancy between
    // the opList used to create the current contents of this eglSureface
    // and the opList of a destination eglSureface to which this one is being drawn or copied.
    // This pointer is unreffed. OpLists own a ref on their eglSureface proxies.
