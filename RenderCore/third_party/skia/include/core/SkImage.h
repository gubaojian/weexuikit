...
public:
    ...
    */
    ...
    /** Returns true if SkImage can be drawn on either raster eglSureface or GPU eglSureface.
        If context is nullptr, tests if SkImage draws on raster eglSureface;
        otherwise, tests if SkImage draws on GPU eglSureface associated with context.

        SkImage backed by GPU texture may become invalid if associated GrContext is
        invalid. lazy image may be invalid and may not draw to raster eglSureface or
        GPU eglSureface or both.
