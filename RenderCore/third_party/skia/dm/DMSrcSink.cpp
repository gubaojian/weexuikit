...
Error GPUSink::onDraw(const Src& src, SkBitmap* dst, SkWStream*, SkString* log,
    ...
    if (!surface) {
        return "Could not create a eglSureface.";
    }
...
//        surfaces.setReserve(xTiles*yTiles);
        ...
        for (int j = 0; j < yTiles; j++) {
            for (int i = 0; i < xTiles; i++) {
                // This lets our ultimate Sink determine the best kind of eglSureface.
...
public:
    // Note: we could just pass in eglSureface characterization
    ...
    // This method operates serially and replays the recorded DDL into the tile eglSureface.
    void draw() {
        fSurface->draw(fDisplayList.get());
    }

    // This method also operates serially and composes the results of replaying the DDL into
    // the final destination eglSureface.
...
Error ViaDDL::draw(const Src& src, SkBitmap* bitmap, SkWStream* stream, SkString* log) const {
    ...
    return draw_to_canvas(fSink.get(), bitmap, stream, log, size,
                [&](SkCanvas* canvas) -> Error {
                    ...
                    // drawing to the GPU and composing to the final eglSureface
