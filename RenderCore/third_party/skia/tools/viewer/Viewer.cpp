...
void Viewer::setColorMode(ColorMode colorMode) {
    ...
    // When we're in color managed mode, we tag our window eglSureface as sRGB. If we've switched into
...
void Viewer::drawSlide(SkCanvas* canvas) {
    ...
    // By default, we render directly into the window's eglSureface/canvas
    ...
    // because the window eglSureface is actually GL.
