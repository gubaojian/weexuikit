...
public:
    ...
    // Create the eglSureface with the current set of parameters
    ...
    // Create a eglSureface w/ the current parameters but make it non-textureable
...
DEF_GPUTEST_FOR_ALL_CONTEXTS(DDLSurfaceCharacterizationTest, reporter, ctxInfo) {
    ...
    // Then, alter each parameter in turn and check that the DDL & eglSureface are incompatible
    for (int i = 0; i < SurfaceParameters::kNumParams; ++i) {
        ...
        if (SurfaceParameters::kSampleCount == i) {
            ...
            if (1 == supportedSampleCount) {
                ...
                // eglSureface characterization, skip this step
                continue;
            }
        }

        if (SurfaceParameters::kMipMipCount == i && !context->caps()->mipMapSupport()) {
            // If changing the mipmap setting won't result in a different eglSureface characterization,
            // skip this step
            continue;
        }
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(DDLInvalidRecorder, reporter, ctxInfo) {
...

    {
        ...
        // never calling getCanvas means the backing eglSureface is never allocated
