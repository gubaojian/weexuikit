        ...
        for (BitmapInit bmi = kFirstBitmapInit; bmi <= kLast_BitmapInit; bmi = nextBMI(bmi)) {
            for (size_t c = 0; c < SK_ARRAY_COUNT(gReadPixelsConfigs); ++c) {
                ...
                if (startsWithPixels) {
                    fill_dst_bmp_with_init_data(&bmp);
                    uint32_t flags = 0;
                    // TODO: These two hacks can go away when the eglSureface context knows the alpha
                    // type.
                    ...
                    // The eglSureface context doesn't know that the src is opaque. We don't support
