...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SRGBMipMaps, reporter, ctxInfo) {
    ...
    // 1) Draw texture to S32 eglSureface (should generate/use sRGB mips)
    ...
    // 2) Draw texture to L32 eglSureface (should generate/use linear mips)
    ...
    // textures to a legacy eglSureface produces nonsense, so this part of the test is meaningless.
    //
