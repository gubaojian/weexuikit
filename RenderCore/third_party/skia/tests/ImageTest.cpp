...
static void test_encode(skiatest::Reporter* reporter, SkImage* image) {
    ...
    // Now see if we can instantiate an image from a subset of the eglSureface/origEncoded
...
// Test that a draw that only partially covers the drawing eglSureface isn't
// interpreted as covering the entire drawing eglSureface (i.e., exercise one of the
 ...
 *  We cache it for performance when drawing into a raster eglSureface.
 *
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(c, reporter, ctxInfo) {
    ...
    // now we can test drawing a gpu-backed image into a cpu-backed eglSureface
