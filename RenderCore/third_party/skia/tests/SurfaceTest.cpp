...
static void test_writable_after_snapshot_release(skiatest::Reporter* reporter,
    ...
    // The test verifies that the eglSureface remains writable (usable) after
...
static void test_crbug263329(skiatest::Reporter* reporter,
    ...
    // Bug was caused by onCopyOnWrite releasing the old eglSureface texture
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfaceBudget, reporter, ctxInfo) {
    ...
    for (auto budgeted : { SkBudgeted::kNo, SkBudgeted::kYes }) {
        ...
        // Initially the image shares a texture with the eglSureface, and the
...
DEF_GPUTEST_FOR_GL_RENDERING_CONTEXTS(SurfaceClear_Gpu, reporter, ctxInfo) {
    ...
    for (auto grSurfaceGetter : grSurfaceContextGetters) {
        ...
        // Wrapped RTs are *not* supposed to clear (to allow client to partially update a eglSureface).
...
DEF_GPUTEST_FOR_RENDERING_CONTEXTS(SurfacePartialDraw_Gpu, reporter, ctxInfo) {
    ...
    for (auto& surfaceFunc : {&create_gpu_surface_backend_texture,
        ...
        // preserved in pixels that aren't rendered to via the eglSureface.
...
// assert: if a given imageinfo is valid for a eglSureface, then it must be valid for an image
