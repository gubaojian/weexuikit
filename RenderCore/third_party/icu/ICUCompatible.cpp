...
#define uc_macro(V) \
    ...
    V(uscript_getScript) \
    V(uscript_hasScript) \
    V(uscript_getScriptExtensions) \
    V(uscript_getShortName) \
    V(u_islower) \
    V(u_getCombiningClass) \
    V(ubrk_isBoundary) \
    V(utext_openUChars) \
    V(ubrk_preceding) \
    V(ubrk_openRules) \
    V(ucnv_close) \
    V(ucnv_countAvailable) \
    V(ucnv_getAvailableName) \
    V(ucnv_getStandardName) \
    V(ucnv_countAliases) \
    V(ucnv_getAlias) \
    V(ucnv_getName) \
    V(ucnv_open) \
    V(ucnv_setFallback) \
    V(ucnv_toUnicode) \
    V(ucnv_setToUCallBack) \
    V(ucnv_setSubstChars) \
    V(ucnv_setFromUCallBack) \
    V(ucnv_fromUnicode) \
    V(unorm_quickCheck) \
    V(u_getIntPropertyValue) \
    V(ucnv_cbFromUWriteBytes) \
    V(ucnv_cbFromUWriteUChars) \
    V(unorm2_composePair) \
    V(unorm2_getRawDecomposition) \
    V(u_countChar32) \
    V(u_strToUTF32) \
    V(unorm2_getNFCInstance) \
    V(UCNV_FROM_U_CALLBACK_ESCAPE) \
    V(UCNV_FROM_U_CALLBACK_SUBSTITUTE) \
    V(UCNV_TO_U_CALLBACK_SUBSTITUTE) \
    V(utext_setup) \
    V(ubidi_open) \
    V(ubidi_setPara) \
    V(ubidi_countRuns) \
    V(ubidi_getVisualRun) \
    V(u_hasBinaryProperty) \
    V(ubidi_close) \
    V(u_strFromUTF8) \
    V(unorm2_getNFDInstance) \
    V(uloc_canonicalize) \
    V(uloc_addLikelySubtags) \
    V(uloc_toLanguageTag) \
    V(ubrk_first) \
    V(uloc_getLanguage) \
