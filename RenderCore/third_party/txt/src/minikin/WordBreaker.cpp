...
void WordBreaker::setLocale(const char* locale) {
  UErrorCode status = U_ZERO_ERROR;
  if(mBreakIterator != nullptr){
     ubrk_close(mBreakIterator);
     mBreakIterator = nullptr;
  }
  mBreakIterator = ubrk_open(UBRK_LINE, locale,  0, -1, &status);
  // TODO: handle failure status
  if (mText != nullptr) {
    ubrk_setUText(mBreakIterator, &mUText, &status);
  }
...
int32_t WordBreaker::iteratorNext() {
  ...
  do {
    if (mIteratorWasReset) {
      result = ubrk_following(mBreakIterator, mCurrent);
      mIteratorWasReset = false;
    } else {
      result = ubrk_next(mBreakIterator);
    }
...
void WordBreaker::detectEmailOrUrl() {
  ...
  if (mLast >= mScanOffset) {
    ...
    if (state == SAW_AT || state == SAW_COLON_SLASH_SLASH) {
      if (!ubrk_isBoundary(mBreakIterator, i)) {
        ...
        i = ubrk_following(mBreakIterator, i);
      }
      mInEmailOrUrl = true;
      mIteratorWasReset = true;
    } else {
