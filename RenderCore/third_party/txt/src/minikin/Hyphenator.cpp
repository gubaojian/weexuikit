...
inline std::string getLanguage(const char* locale){
  UErrorCode	icuStatus = U_ZERO_ERROR;
  int32_t		languageLength = 0;
  char		languageChars[ULOC_LANG_CAPACITY + ULOC_FULLNAME_CAPACITY];
  languageLength = uloc_getLanguage(locale, languageChars, ULOC_LANG_CAPACITY, &icuStatus);
  std::string string;
  string.append(languageChars, languageLength);
  return string;
}
