...
MessageLoopAndroid::MessageLoopAndroid()
  ...
  WML_CHECK(add_result == 1);
}

MessageLoopAndroid::~MessageLoopAndroid() {
  int remove_result = ::ALooper_removeFd(looper_.get(), timer_fd_.get());
  WML_CHECK(remove_result == 1);
}

void MessageLoopAndroid::Run() {
  WML_DCHECK_(looper_.get() == ALooper_forThread());
...
void MessageLoopAndroid::WakeUp(wml::TimePoint time_point) {
