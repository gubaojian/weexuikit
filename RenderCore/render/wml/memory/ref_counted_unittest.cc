...
class MyClass : public RefCountedThreadSafe<MyClass> {
 ...
 private:
  WML_FRIEND_REF_COUNTED_THREAD_SAFE(MyClass);
  WML_FRIEND_MAKE_REF_COUNTED(MyClass);

  bool* was_destroyed_;

  WML_DISALLOW_COPY_AND_ASSIGN(MyClass);
};

class MySubclass final : public MyClass {
 private:
  WML_FRIEND_REF_COUNTED_THREAD_SAFE(MySubclass);
  WML_FRIEND_MAKE_REF_COUNTED(MySubclass);
  ...
  WML_DISALLOW_COPY_AND_ASSIGN(MySubclass);
};
...
class MyPublicClass : public RefCountedThreadSafe<MyPublicClass> {
 ...
 private:
  ...
  WML_DISALLOW_COPY_AND_ASSIGN(MyPublicClass);
};
