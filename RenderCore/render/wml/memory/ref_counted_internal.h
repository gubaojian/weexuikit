...
#ifndef WEEX_UIKIT_WML_MEMORY_REF_COUNTED_INTERNAL_H_
#define WEEX_UIKIT_WML_MEMORY_REF_COUNTED_INTERNAL_H_
...
class RefCountedThreadSafeBase {
 public:
  void AddRef() const {
#ifndef NDEBUG
    WML_DCHECK_(!adoption_required_);
    WML_DCHECK_(!destruction_started_);
#endif
  ...
  void AssertHasOneRef() const { WML_DCHECK_(HasOneRef()); }
 ...
 protected:
  ...
  bool Release() const {
    ...
    WML_DCHECK_(!destruction_started_);
#endif
    WML_DCHECK_(ref_count_.load(std::memory_order_acquire) != 0u);
 ...
 private:
  ...
  WML_DISALLOW_COPY_AND_ASSIGN(RefCountedThreadSafeBase);
};
...
#endif  // WEEX_UIKIT_WML_MEMORY_REF_COUNTED_INTERNAL_H_
