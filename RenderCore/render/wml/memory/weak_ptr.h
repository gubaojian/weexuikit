...
#ifndef WEEX_UIKIT_WML_MEMORY_WEAK_PTR_H_
#define WEEX_UIKIT_WML_MEMORY_WEAK_PTR_H_

#include <utility>

#include "wml/logging.h"
#include "wml/memory/ref_counted.h"
#include "wml/memory/thread_checker.h"
#include "wml/memory/weak_ptr_internal.h"

namespace wml {

struct DebugThreadChecker {
  WML_DECLARE_THREAD_CHECKER(checker);
};
...
class WeakPtr {
 public:
  ...
  explicit operator bool() const {
    WML_DCHECK_CREATION_THREAD_IS_CURRENT(checker_.checker);
 ...
 private:
  ...
  explicit WeakPtr(T* ptr,
                   wml::RefPtr<wml::internal::WeakPtrFlag>&& flag,
                   DebugThreadChecker checker)
      : ptr_(ptr), flag_(std::move(flag)), checker_(checker) {}

  T* ptr_;
  wml::RefPtr<wml::internal::WeakPtrFlag> flag_;
...
class WeakPtrFactory {
 public:
  explicit WeakPtrFactory(T* ptr)
      : ptr_(ptr), flag_(wml::MakeRefCounted<wml::internal::WeakPtrFlag>()) {
    WML_DCHECK_(ptr_);
  }
 ...
 private:
  ...
  WML_DISALLOW_COPY_AND_ASSIGN(WeakPtrFactory);
};

}  // namespace wml

#endif  // WEEX_UIKIT_WML_MEMORY_WEAK_PTR_H_
