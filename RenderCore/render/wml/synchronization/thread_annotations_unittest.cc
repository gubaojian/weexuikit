...
class GuardedByClass {
 ...
 private:
  std::mutex mu_;
  int x_ WML_GUARDED_BY(mu_);

  WML_DISALLOW_COPY_AND_ASSIGN(GuardedByClass);
};
...
class AcquiredBeforeClass1 {
 ...
 private:
  ...
  WML_DISALLOW_COPY_AND_ASSIGN(AcquiredBeforeClass1);
};
...
class AcquiredBeforeClass2 {
 ...
 private:
  std::mutex mu_ WML_ACQUIRED_BEFORE(AcquiredBeforeClass1::mu_);

  WML_DISALLOW_COPY_AND_ASSIGN(AcquiredBeforeClass2);
};
