...
#ifndef WEEX_UIKIT_GESTURE_SCROLLPOSITION_H
#define WEEX_UIKIT_GESTURE_SCROLLPOSITION_H
#include <algorithm>
#include <memory>
...
namespace weexuikit {
    class ScrollPosition : public ViewportOffset, public ScrollMetrics {
    ...
    public:
        ...
        ScrollPosition(ScrollPhysics *physics, ScrollContext *context, bool keepScrollOffset,
            ...
            this->mActivity = nullptr;
            this->keepScrollOffset = keepScrollOffset;
            this->mMaxScrollExtent = 0;
            this->mMinScrollExtent = 0;
            this->mPixels = 0;
    ...
    public:
        ...
        double viewportDimension() override {
        ...
        virtual void absorb(ScrollPosition *other) {
            ...
            mMinScrollExtent = other->minScrollExtent();
            mMaxScrollExtent = other->maxScrollExtent();
            mPixels = other->mPixels;
            mViewportDimension = other->viewportDimension();

            mActivity = other->activity();
            other->mActivity = nullptr;
            if (other != this)
                activity()->resetActivity();
            context->setIgnorePointer(activity()->shouldIgnorePointer());
            isScrollingNotifier.setValue(activity()->isScrolling());
        }
        ...
        double setPixels(double newPixels) {
            if (newPixels != pixels()) {
                ...
                double oldPixels = mPixels;
                mPixels = newPixels - overscroll;
                if (mPixels != oldPixels) {
                    notifyListeners();
                    didUpdateScrollPositionBy(mPixels - oldPixels);
                }
                if (overscroll != 0) {
        ...
        virtual void correctBy(double correction) override {
            mPixels += correction;
            mDidChangeViewportDimensionOrReceiveCorrection = true;
        }
        ...
        virtual bool applyViewportDimension(double viewportDimension) override {
            if (mViewportDimension != viewportDimension) {
                mViewportDimension = viewportDimension;
                mDidChangeViewportDimensionOrReceiveCorrection = true;
        ...
        virtual bool
        applyContentDimensions(double minScrollExtent, double maxScrollExtent) override {
            if (!nearEqual(mMinScrollExtent, minScrollExtent,
                           Tolerance::getDefaultTolerance()->distance) ||
                !nearEqual(mMaxScrollExtent, maxScrollExtent,
                           Tolerance::getDefaultTolerance()->distance) ||
                mDidChangeViewportDimensionOrReceiveCorrection) {
                mMinScrollExtent = minScrollExtent;
                mMaxScrollExtent = maxScrollExtent;
                mHaveDimensions = true;
                applyNewDimensions();
                mDidChangeViewportDimensionOrReceiveCorrection = false;
            }
            return true;
        }
        ...
        virtual void animateTo(double to, int64_t duration, Curve &curve) override =0;
        ...
        virtual void jumpTo(double value) override =0;

        virtual bool allowImplicitScrolling() override {
            return physics->allowImplicitScrolling();
        }

        /// Deprecated. Use [jumpTo] or a custom [ScrollPosition] instead.
        virtual void jumpToWithoutSettling(double value)=0;

        /// Stop the current activity and start a [HoldScrollActivity].
        virtual std::shared_ptr<ScrollHoldController> hold(VoidCallback holdCancelCallback)=0;
        ...
        virtual std::shared_ptr<Drag>
        drag(DragStartDetails details, VoidCallback dragCancelCallback)=0;
        ...
        std::shared_ptr<ScrollActivity> activity() {
            return mActivity;
        }
        ...
        virtual void beginActivity(std::shared_ptr<ScrollActivity> newActivity) {
            if (newActivity.get() == nullptr) {
                return;
            }
            bool wasScrolling, oldIgnorePointer;
            if (mActivity.get() != nullptr) {
                oldIgnorePointer = mActivity->shouldIgnorePointer();
                wasScrolling = mActivity->isScrolling();
            ...
            if (oldIgnorePointer != activity()->shouldIgnorePointer())
                context->setIgnorePointer(activity()->shouldIgnorePointer());
            isScrollingNotifier.setValue(activity()->isScrolling());
            if (!wasScrolling && mActivity->isScrolling())
                didStartScroll();
        }
        ...
        void didStartScroll() {
            FixedScrollMetrics fixedScrollMetrics = FixedScrollMetrics(minScrollExtent(),
                                                                       maxScrollExtent(), pixels(),
                                                                       viewportDimension(),
                                                                       axisDirection());
            activity()->dispatchScrollStartNotification(&fixedScrollMetrics);
        }
        ...
        void didUpdateScrollPositionBy(double delta) {
            FixedScrollMetrics fixedScrollMetrics = FixedScrollMetrics(minScrollExtent(),
                                                                       ...
                                                                       viewportDimension(),
                                                                       axisDirection());
            activity()->dispatchScrollUpdateNotification(&fixedScrollMetrics, delta);
        }
        ...
        void didEndScroll() {
            FixedScrollMetrics fixedScrollMetrics = FixedScrollMetrics(minScrollExtent(),
                                                                       ...
                                                                       viewportDimension(),
                                                                       axisDirection());
            activity()->dispatchScrollEndNotification(&fixedScrollMetrics);
            if (keepScrollOffset)
                saveScrollOffset();
        }
        ...
        void didOverscrollBy(double value) {
            ...
            FixedScrollMetrics fixedScrollMetrics = FixedScrollMetrics(minScrollExtent(),
                                                                       ...
                                                                       viewportDimension(),
                                                                       axisDirection());
            activity()->dispatchOverscrollNotification(&fixedScrollMetrics, value);
        }
        ...
        void didUpdateScrollDirection(ScrollDirection direction) {
            //FixedScrollMetrics fixedScrollMetrics = FixedScrollMetrics(minScrollExtent(), maxScrollExtent(), pixels(), viewportDimension(), axisDirection());
            //UserScrollNotification(&fixedScrollMetrics, context->notificationContext(), direction).dispatch(context->notificationContext());
        }
        ...
        virtual void dispose() {
            if (activity() != nullptr) {
                mActivity->dispose();
                mActivity.reset(); // it will be null if it got absorbed by another ScrollPosition
            }
        }


        virtual void notifyListeners() override {
            updateSemanticActions(); // will potentially request a semantics update.
            ViewportOffset::notifyListeners();
        }


        virtual bool isScrollPositionWithSingleContext() {
            return false;
        }

    public:
        std::shared_ptr<ScrollActivity> mActivity;
        double mPixels;
        double mMinScrollExtent;
        double mMaxScrollExtent;
        bool mDidChangeViewportDimensionOrReceiveCorrection = true;
...
#endif //WEEX_UIKIT_GESTURE_SCROLLPOSITION_H
