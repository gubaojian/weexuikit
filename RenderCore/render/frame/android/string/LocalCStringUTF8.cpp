 ...
 * under the License.
 */
#include "LocalCStringUTF8.h"

namespace weexuikit {

LocalCStringUTF8::LocalCStringUTF8(JNIEnv *env, jstring _jstring)
        : m_env(env), m_jstring(_jstring), m_chars(nullptr) {
}

LocalCStringUTF8::~LocalCStringUTF8() {
  if (m_chars)
    m_env->ReleaseStringUTFChars(m_jstring, m_chars);
}

const char *LocalCStringUTF8::getChars() {
  if (m_chars)
    return m_chars;
