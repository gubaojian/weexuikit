...
{
  return renderer()->isRelPositioned() ? renderer()->offsetForInFlowPosition() : LayoutSize();
}
...
LayoutPoint RenderLayer::location() const {
  ...
  if (renderer()->isOutOfFlowPositioned() && enclosingPositionedAncestor()) {
    ...
    if (positionedParent->renderer()->isRelPositioned() && positionedParent->renderer()->isRenderInline()) {
      LayoutSize offset = toRenderInline(positionedParent->renderer())->offsetForInFlowPositionedInline(renderer());
      localPoint += offset;
    }
  }
...
static bool inContainingBlockChain(RenderLayer* startLayer, RenderLayer* endLayer)
{
  if (startLayer == endLayer)
    return true;
  for (RenderBlock* currentBlock = startLayer->renderer()->containingBlock(); currentBlock && !currentBlock->isRenderView(); currentBlock = currentBlock->containingBlock()) {
