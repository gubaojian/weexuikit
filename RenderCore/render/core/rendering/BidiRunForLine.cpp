...
static void restoreIsolatedMidpointStates(
    InlineBidiResolver& topResolver,
  ...
  while (!isolatedResolver.isolatedRuns().isEmpty()) {
    ...
    topResolver.setMidpointStateForIsolatedRun(
        run, isolatedResolver.midpointStateForIsolatedRun(run));
  }
}
...
void constructBidiRunsForLine(InlineBidiResolver& topResolver,
  ...
  while (!topResolver.isolatedRuns().isEmpty()) {
    ...
    isolatedLineMidpointState =
        topResolver.midpointStateForIsolatedRun(isolatedRun);
    ...
    isolatedResolver.setStatus(
        statusWithDirection(direction, isOverride(unicodeBidi)));
    ...
    InlineIterator iter =
        InlineIterator(isolatedInline, startObj, isolatedRun->m_start);
