...
LayoutPoint RenderBoxModelObject::adjustedPositionRelativeToOffsetParent(
  ...
  RenderObject* offsetParent = offsetParentRenderObject();
  if(offsetParent == nullptr){
    return referencePoint;
  }

  if (const RenderBoxModelObject* offsetParent = offsetParent) {
...
void RenderBoxModelObject::calculateBackgroundImageGeometry(const RenderBox* paintContainer, const FillLayer& fillLayer, const LayoutRect& paintRect,
      ...
      if (!fixedAttachment) {
        ...
        positioningAreaSize = pixelSnappedIntSize(paintRect.size() - LayoutSize(left + right, top + bottom), paintRect.location());
         ...
         //FIXED Style Fixed Current Is Not Supported, background is always to be local
      }
...
static inline IntSize resolveAgainstIntrinsicRatio(
  ...
  // rect with the largest eglSureface area.
