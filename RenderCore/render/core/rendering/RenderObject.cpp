...
#include "render/platform/geometry/TransformState.h"
#include "render/platform/graphics/GraphicsContext.h"
#include "render/wtf/RefCountedLeakCounter.h"
#include "render/wtf/text/StringBuilder.h"
...
struct SameSizeAsRenderObject {
  ...
  weexuikit::Node*  ptr;
  weexuikit::PointerEventListener* pointerEventListener;
};
...
RenderObject* RenderObject::offsetParentRenderObject() const
{
  for (RenderObject* ancestor = parent(); ancestor; ancestor = ancestor->parent()) {
    // Spec: http://www.w3.org/TR/cssom-view/#offset-attributes
    weexuikit::Node* node = ancestor->getNode();
    if (!node)
      continue;
...
void RenderObject::updateHitTestResult(HitTestResult& result,
  ...
  if(result.hasRenderObject()){
     return;
  }
  result.setRenderObject(this);
}

/**
 * check whether, is an hit test path
 * */
bool RenderObject::nodeAtPoint(const HitTestRequest& request,
                               HitTestResult&,
                               const HitTestLocation& locationInContainer/*locationInContainer*/,
                               const LayoutPoint& accumulatedOffset/*accumulatedOffset*/) {
  ...
  if(request.requestEvent() == weexuikit::Event::RequestEventType::ACCESSIBILITY){
    if(node->hasAccessibility() && !RenderObject::isParentAriaHidden(this)){
       return true;
    }
    return false;
  }

  if(mPointerEventListener == nullptr){
      return false;
  }
  return true;
}

bool RenderObject::isParentAriaHidden(RenderObject* renderObject){
  if(renderObject == nullptr){
     return false;
  }
  if(renderObject->getNode() != nullptr){
    if(renderObject->getNode()->ariaHidden()){
       return true;
    }
  }
  return RenderObject::isParentAriaHidden(renderObject->parent());
}
