...
bool RenderBox::nodeAtPoint(const HitTestRequest& request,
  ...
  if (visibleToHitTestRequest(request) &&
    ...
    if(RenderObject::nodeAtPoint(request, result, locationInContainer,
                                 adjustedLocation)){
      updateHitTestResult(
              result, locationInContainer.point() - toLayoutSize(adjustedLocation));
      return true;
...
void RenderBox::computeLogicalHeight(
  ...
  if (isOutOfFlowPositioned()) {
    ...
    if (hasOverrideHeight() && (parent()->isFlexibleBox() || parent()->isViewPort()))
...
LayoutUnit RenderBox::computeIntrinsicLogicalContentHeightUsing(
  ...
  if (logicalHeightLength.isMinContent() ||
    ...
    LayoutUnit intrinsicHeight = -1;
    if (isReplaced()){
       intrinsicHeight = intrinsicSize().height();
    }else if(m_intrinsicContentLogicalHeight != -1){
      intrinsicHeight = m_intrinsicContentLogicalHeight;
    }else{
      intrinsicHeight = intrinsicContentHeight;
    }

    if (style()->boxSizing() == BORDER_BOX && intrinsicHeight.toInt() > 0) {
      return intrinsicHeight + borderAndPadding;
    }
