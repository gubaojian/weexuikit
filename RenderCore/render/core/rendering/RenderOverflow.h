...
#ifndef WEEX_UIKIT_CORE_RENDERING_RENDEROVERFLOW_H_
#define WEEX_UIKIT_CORE_RENDERING_RENDEROVERFLOW_H_

#include "render/platform/geometry/LayoutRect.h"

namespace blink {
// RenderOverflow is a class for tracking content that spills out of a box.
// This class is used by RenderBox and InlineFlowBox.
//
// There are two types of overflow: layout overflow (which is expected to be
// reachable via scrolling mechanisms) and visual overflow (which is not
// expected to be reachable via scrolling mechanisms).
//
// Layout overflow examples include other boxes that spill out of our box,  For
// example, in the inline case a tall image could spill out of a line box.

// Examples of visual overflow are shadows, text stroke (and eventually outline
// and border-image).

// This object is allocated only when some of these fields have non-default
// values in the owning box.
...
#endif  // WEEX_UIKIT_CORE_RENDERING_RENDEROVERFLOW_H_
