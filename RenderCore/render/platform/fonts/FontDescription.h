...
#ifndef WEEX_UIKIT_PLATFORM_FONTS_FONTDESCRIPTION_H_
#define WEEX_UIKIT_PLATFORM_FONTS_FONTDESCRIPTION_H_

#include "render/platform/fonts/FixedPitchFontType.h"
#include "render/platform/fonts/FontCacheKey.h"
#include "render/platform/fonts/FontFamily.h"
#include "render/platform/fonts/FontFeatureSettings.h"
#include "render/platform/fonts/FontOrientation.h"
#include "render/platform/fonts/FontSmoothingMode.h"
#include "render/platform/fonts/FontTraits.h"
#include "render/platform/fonts/FontWidthVariant.h"
#include "render/platform/fonts/TextRenderingMode.h"
#include "render/platform/fonts/TypesettingFeatures.h"
#include "render/platform/text/NonCJKGlyphOrientation.h"
...
class PLATFORM_EXPORT FontDescription {
 public:
  ...
  GenericFamilyType genericFamily() const {
    return static_cast<GenericFamilyType>(m_genericFamily);
  }
  ...
  LigaturesState commonLigaturesState() const {
    return static_cast<LigaturesState>(m_commonLigaturesState);
  }
  LigaturesState discretionaryLigaturesState() const {
    return static_cast<LigaturesState>(m_discretionaryLigaturesState);
  }
  LigaturesState historicalLigaturesState() const {
    return static_cast<LigaturesState>(m_historicalLigaturesState);
  }
  LigaturesState contextualLigaturesState() const {
    return static_cast<LigaturesState>(m_contextualLigaturesState);
  }
  FontSmoothingMode fontSmoothing() const {
    return static_cast<FontSmoothingMode>(m_fontSmoothing);
  }
  TextRenderingMode textRendering() const {
    return static_cast<TextRenderingMode>(m_textRendering);
  }
  ...
  FontOrientation orientation() const {
    return static_cast<FontOrientation>(m_orientation);
  }
  ...
  FontWidthVariant widthVariant() const {
    return static_cast<FontWidthVariant>(m_widthVariant);
  }
  ...
  void setFontSmoothing(FontSmoothingMode smoothing) {
    m_fontSmoothing = smoothing;
  }
  void setTextRendering(TextRenderingMode rendering) {
    m_textRendering = rendering;
    updateTypesettingFeatures();
  }
  ...
  void setNonCJKGlyphOrientation(NonCJKGlyphOrientation orientation) {
    m_nonCJKGlyphOrientation = orientation;
  }
  ...
  TypesettingFeatures typesettingFeatures() const {
    return static_cast<TypesettingFeatures>(m_typesettingFeatures);
  }
 ...
 private:
  ...
  float m_specifiedSize;  // Specified CSS value. Independent of rendering
                          // issues such as integer rounding, minimum font
                          // sizes.
  ...
  unsigned m_orientation : 1;  // FontOrientation - Whether the font is
                               // rendering on a horizontal line or a vertical
                               // line.
  unsigned m_nonCJKGlyphOrientation : 1;  // NonCJKGlyphOrientation - Only used
                                          // by vertical text. Determines the
                                          // default orientation for
                                          // non-ideograph glyphs.
  ...
  unsigned m_script : 7;  // Used to help choose an appropriate font for generic
                          // font families.
...
inline bool FontDescription::operator==(const FontDescription& other) const {
  return m_familyList == other.m_familyList &&
         ...
         m_wordSpacing == other.m_wordSpacing && m_style == other.m_style &&
         m_variant == other.m_variant &&
         m_isAbsoluteSize == other.m_isAbsoluteSize &&
         m_weight == other.m_weight && m_stretch == other.m_stretch &&
         ...
         m_widthVariant == other.m_widthVariant && m_script == other.m_script &&
...
#endif  // WEEX_UIKIT_PLATFORM_FONTS_FONTDESCRIPTION_H_
