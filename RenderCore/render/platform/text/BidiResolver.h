...
#ifndef WEEX_UIKIT_PLATFORM_TEXT_BIDIRESOLVER_H_
#define WEEX_UIKIT_PLATFORM_TEXT_BIDIRESOLVER_H_

#include "render/platform/text/BidiCharacterRun.h"
#include "render/platform/text/BidiContext.h"
#include "render/platform/text/BidiRunList.h"
#include "render/platform/text/TextDirection.h"
#include "render/wtf/HashMap.h"
#include "render/wtf/Noncopyable.h"
#include "render/wtf/PassRefPtr.h"
...
class MidpointState {
 public:
  ...
  // boxes such as RenderInlines or hard line breaks to ensure that they're not
  // ignored.
  ...
  void ensureCharacterGetsLineBox(Iterator& textParagraphSeparator) {
    startIgnoringSpaces(Iterator(0, textParagraphSeparator.object(),
                                 textParagraphSeparator.offset() - 1));
    stopIgnoringSpaces(Iterator(0, textParagraphSeparator.object(),
  ...
  void checkMidpoints(Iterator& lBreak) {
    ...
    if (lBreak.object() && m_numMidpoints && !(m_numMidpoints % 2)) {
      ...
      if (currpoint == lBreak) {
        ...
        // point.
        m_numMidpoints--;
        if (endpoint.object()->style()->collapseWhiteSpace() &&
            endpoint.object()->isText())
...
struct BidiStatus {
  ...
  BidiStatus(WTF::Unicode::Direction eorDir,
...
class BidiResolver {
 ...
 public:
  ...
  // time. But when we layout each isolated span, we will walk into (and ignore)
  // all child isolated spans.
...
template <class Iterator, class Run>
void BidiResolver<Iterator, Run>::embed(WTF::Unicode::Direction dir,
...
template <class Iterator, class Run>
void BidiResolver<Iterator, Run>::lowerExplicitEmbeddingLevel(
...
template <class Iterator, class Run>
void BidiResolver<Iterator, Run>::raiseExplicitEmbeddingLevel(
    BidiRunList<Run>& runs,
    WTF::Unicode::Direction from,
    WTF::Unicode::Direction to) {
  using namespace WTF::Unicode;
...
bool BidiResolver<Iterator, Run>::commitExplicitEmbedding(
  ...
  for (size_t i = 0; i < m_currentExplicitEmbeddingSequence.size(); ++i) {
    ...
    if (embedding.direction() == PopDirectionalFormat) {
      ...
      Direction direction = (embedding.direction() == RightToLeftEmbedding ||
...
template <class Iterator, class Run>
inline void BidiResolver<Iterator, Run>::updateStatusLastFromCurrentDirection(
...
template <class Iterator, class Run>
TextDirection BidiResolver<Iterator, Run>::determineParagraphDirectionality(
...
template <class Iterator, class Run>
void BidiResolver<Iterator, Run>::createBidiRunsForLine(
    const Iterator& end,
...
template <class Iterator, class Run>
void BidiResolver<Iterator, Run>::setMidpointStateForIsolatedRun(
    Run* run,
...
BidiResolver<Iterator, Run>::midpointStateForIsolatedRun(Run* run) {
  return m_midpointStateForIsolatedRun.take(run);
}

}  // namespace blink

#endif  // WEEX_UIKIT_PLATFORM_TEXT_BIDIRESOLVER_H_
