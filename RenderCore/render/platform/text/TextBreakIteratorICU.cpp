...
#include "render/platform/text/TextBreakIteratorInternalICU.h"
#include "render/wtf/Assertions.h"
#include "render/wtf/HashMap.h"
#include "render/wtf/PassOwnPtr.h"
#include "render/wtf/ThreadSpecific.h"
#include "render/wtf/ThreadingPrimitives.h"
#include "render/wtf/text/AtomicString.h"
#include "render/wtf/text/CString.h"
...
namespace blink {
  ...
  static TextBreakIterator* createTextBreakIterator(UBreakIteratorType type, UErrorCode* openStatus, const char* local){
    if(!local){
      local = currentTextBreakLocaleID();
    }
    UBreakIterator* iterator = ubrk_open(type, local, 0, -1, openStatus);
    if (!iterator)
       return nullptr;
    TextBreakIterator* textBreakIterator = new TextBreakIterator(iterator);
    return textBreakIterator;
  }
 ...
 public:
  ...
  TextBreakIterator* take(const AtomicString& locale) {
    ...
    if (!iterator) {
      ...
      iterator = createTextBreakIterator(UBRK_LINE, &openStatus, localeIsEmpty ? currentTextBreakLocaleID() : locale.utf8().data());
      ...
      if (!localeIsEmpty && U_FAILURE(openStatus)) {
        ...
        iterator = createTextBreakIterator(UBRK_LINE, &openStatus, nullptr);
      }
...
static TextBreakIterator* wordBreakIterator(const LChar* string, int length) {
  ...
  if (!breakIter) {
    breakIter = createTextBreakIterator(UBRK_WORD, &errorCode, nullptr);
...
TextBreakIterator* wordBreakIterator(const UChar* string, int length) {
  ...
  if (!breakIter) {
    UErrorCode openStatus = U_ZERO_ERROR;
    UBreakIterator* iterator = ubrk_open(UBRK_WORD, currentTextBreakLocaleID(), 0, 0, &openStatus);
    ASSERT_WITH_MESSAGE(U_SUCCESS(openStatus), "ICU could not open a break iterator: %s (%d)", u_errorName(openStatus), openStatus);
    if (!iterator)
       return nullptr;
    breakIter = new TextBreakIterator(iterator);
    if (!breakIter)
      return 0;
  }
...
void NonSharedCharacterBreakIterator::createIteratorForBuffer(
  ...
  if (!createdIterator) {
    UErrorCode errorCode = U_ZERO_ERROR;
    m_iterator = createTextBreakIterator(UBRK_CHARACTER, &errorCode, nullptr);
    ASSERT_WITH_MESSAGE(U_SUCCESS(errorCode),
                        "ICU could not open a break iterator: %s (%d)",
                        u_errorName(errorCode), errorCode);
  }
...
TextBreakIterator* sentenceBreakIterator(const UChar* string, int length) {
  ...
  if (!iterator) {
    iterator = createTextBreakIterator(UBRK_SENTENCE, &openStatus, nullptr);
...
bool isWordTextBreak(TextBreakIterator* iterator) {
  int ruleStatus = ubrk_getRuleStatus(iterator->getUBreakIterator());
...
static TextBreakIterator* setUpIteratorWithRules(const char* breakRules,
  ...
  if (!iterator) {
    ...
    UBreakIterator* ubit = 0;
    ubit = ubrk_openRules(rules.data(), rules.size(), string, length, &parseStatus, &openStatus);
    ASSERT_WITH_MESSAGE(U_SUCCESS(openStatus),
                        "ICU could not open a break iterator: %s (%d)",
                        u_errorName(openStatus), openStatus);
    if (!ubit)
      return 0;
    iterator->setUBreakIterator(ubit);
  }
