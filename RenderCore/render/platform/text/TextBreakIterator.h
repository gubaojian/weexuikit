...
#ifndef WEEX_UIKIT_PLATFORM_TEXT_TEXTBREAKITERATOR_H_
#define WEEX_UIKIT_PLATFORM_TEXT_TEXTBREAKITERATOR_H_
...
public:
    TextBreakIterator(UBreakIterator* iterator){
      m_iterator = iterator;
    };

    void reset(const UChar* str, int len){
       UErrorCode status;
      ubrk_setText(m_iterator, str, len, &status);
    }

    int32_t first(){
      return ubrk_first(m_iterator);
    }
    int32_t last(){
      return ubrk_last(m_iterator);
    }
    ...
    int32_t previous(){
      return ubrk_previous(m_iterator);
    }
    int32_t following(int32_t position){
      return ubrk_following(m_iterator, position);
    }
    int32_t preceding(int32_t position){
      return ubrk_preceding(m_iterator, position);
    }

    bool isBoundary(int32_t position){
      return ubrk_isBoundary(m_iterator, position);
    }

    int32_t current(void) const{
      return ubrk_current(m_iterator);
    }

    UBreakIterator* getUBreakIterator(){
      return m_iterator;
    }
    void setUBreakIterator(UBreakIterator* uit){
       m_iterator = uit;
    }

    void setText(UText *text, UErrorCode* status){
...
#endif  // WEEX_UIKIT_PLATFORM_TEXT_TEXTBREAKITERATOR_H_
